...
static struct used_atom {
	...
	union {
		...
		struct {
			enum { RAW_BARE, RAW_LENGTH, RAW_FILTER, RAW_TEXT_CONV } option;
		} raw_data;
		struct {
				...
				const char *arg, struct strbuf *err)
{
	if (!arg)
		...
		atom->u.raw_data.option = RAW_FILTER;
...
/* See grab_values */
static int grab_raw_data(struct ref_array_item *ref, int deref, void *buf, unsigned long buf_size,
			  struct object *obj, struct expand_data *data, struct strbuf *err)
{
	int i;
	...
	for (i = 0; i < used_atom_cnt; i++) {
		...
		if (starts_with(name, "raw")) {
			...
			} else if (atom->u.raw_data.option == RAW_FILTER) {
				if (!ref->rest)
			...
			struct expand_data *data, struct strbuf *err)
{
	...
	case OBJ_TAG:
		...
		if ((ret = grab_raw_data(ref, deref, buf, buf_size, obj, data, err)))
			return ret;
		      ...
		      struct expand_data *oi, struct strbuf *err)
{
	...
	if (oid_object_info_extended(the_repository, &oi->oid, &oi->info,
		...
		return strbuf_addf_ret(err, -2, _("%s missing\n"), oid_to_hex(&oi->oid));
	...
	if (oi->info.contentp) {
		*obj = parse_object_buffer(the_repository, &oi->oid, oi->type, oi->size, oi->content, &eaten);
		...
		if((ret = grab_values(ref, deref, *obj, oi, err)))
			return ret;
	}
